			...
			$(document).delegate("#saveRegister", "click", function() {
				...
				$('.fee-list li:visible span:not(":last")').each(function() {
					...
					var ms = {
						...
						feeItemId: parseInt($(this).attr('data-feeCatlogId')),
						feeItemName: $(this).attr('data-feeCatlogName'),
				...
				$.ajax(common.ajaxCall({
					...
					success: function(data) {
						...
						art.dialog.open(url + '&patientId=' + data.ds.patientId + '&treateBillNo=' + 
						data.ds.treateBillNo, {
							width: 380,
							height: 420,
							lock: true,
							title: '确认收费'
	...
	var Register = {
		...
		linkage: function(cardType, cardNum, date, age) {
			...
			$.ajax(common.ajaxCall({
				url: api,
				data: {
					cardType: $('#' + cardType).children('option:selected').val(),
					idNumber: num
				},
				type: 'post',
				success: function() {
